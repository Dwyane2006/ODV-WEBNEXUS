...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('a[href^="#"]').forEach(anchor => {
        anchor.addEventListener('click', function(e) {
            ...
            // If target doesn't exist on this page (e.g. #sdg-projects on about.html),
            // let the browser navigate to index.html#sdg-projects normally
        });
    });

    // Handle SDG nav link on non-index pages: redirect to index.html#sdg-projects
    document.querySelectorAll('a[href="#sdg-projects"]').forEach(link => {
        link.addEventListener('click', function(e) {
            const sdgSection = document.getElementById('sdg-projects');
            if (!sdgSection) {
                // Not on index.html — navigate to the section on the home page
                e.preventDefault();
                window.location.href = 'index.html#sdg-projects';
            }
            // If on index.html, the smooth scroll handler above handles it
        });
    });
...
(function() {
    ...
    function renderCards(filter) {
        ...
        grid.querySelectorAll('.sdg-card-image').forEach(imgWrap => {
            const open = () => openLightbox(imgWrap.dataset.img, imgWrap.dataset.title);
    ...
    function openLightbox(src, title) {
        ...
        const label = document.getElementById('sdgLightboxTitle');
        if (!lb) return;
        img.src   = src;
        img.alt   = title;
